...
local assets = {
	Repentance = LoadCustomInstance("https://github.com/AlperPro/shhh/blob/main/miscrepentance.rbxm?raw=true")
}
...
function CrucifixEntity(entityTable, tool)
	...
	if not resist then
		...
		warn("Not Use Solara"):ShakeOnce(7.5, 7.5, 0.25, 1.5)
	else
						...
						do
							...
							if c.Enabled then
								...
								if mag <= c.Range then
									...
									warn("Not Use Solara"):ShakeOnce(table.unpack(cloned))
								end
							end
						end
					end
